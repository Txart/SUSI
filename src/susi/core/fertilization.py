...
class Fertilization:
    ...
    def ph_effect(self, yr):
        tfert = yr - self.spara.fertilization.application_year
    ...
    def nutrient_release(self, yr):
        ...
        if tfert >= 0:
            for nutr in ["N", "P", "K"]:
                fertilization_nutrient = getattr(self.spara.fertilization, nutr)
                ...
                self.release[nutr] = (
                    (
                        dose * np.exp(-decay_k * tfert)
                        - dose * np.exp(-decay_k * (tfert + 1))
                    )
                    * nut_efficiency
                    * np.ones(self.ncols)
