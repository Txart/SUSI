...
class StemCurve:
    def __init__(self):
        ...
        self.lengths = {
            "log": {
                1: (0, 0, 0, 0, 0, 4.3, 4.6, 4.9, 5.2, 5.5, 5.8, 6.1),
                2: (0, 0, 0, 0, 0, 4.3, 4.6, 4.9, 5.2, 5.5, 5.8, 6.1),
                3: (0, 3.1, 3.4, 3.7, 4.0, 4.3, 4.6, 4.9, 5.2, 5.5, 5.8, 6.1),
            },
            "pulp": {
                1: (0, 3.0, 3.3, 3.6, 3.9, 4.2, 4.5, 4.8, 5.1, 5.4, 5.7, 6.0),
    ...
    def buckStem(self, d13, h, sp):
        ...
        if log_flag == 1:
            ...
            for log in self.lengths["log"][sp]:
        ...
        if pulp_flag == 1:
            ...
            for log in self.lengths["pulp"][sp]:
    ...
    def predictAssortmentVolumes(self, d13, h, sp):
        ...
        volume["log"] = self.calculateVolume(
            d13, h, sp, cutPoints["stump"][0], cutPoints["log"][0]
        )
        volume["pulp"] = self.calculateVolume(
            d13, h, sp, cutPoints["log"][0], cutPoints["pulp"][0]
        )
        volume["residual"] = self.calculateVolume(
            d13, h, sp, 0, cutPoints["stump"][0]
        ) + self.calculateVolume(d13, h, sp, cutPoints["pulp"][0], h)
        volume["total"] = volume["log"] + volume["pulp"] + volume["residual"]
        volume["residual"] = volume["residual"] - self.calculateVolume(
            d13, h, sp, 0, cutPoints["stump"][0]
        )
        return volume
