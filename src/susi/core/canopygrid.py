...
class CanopyGrid:
    def __init__(
        self, cpara: CanopyParameters, state: CanopyStateParametersArray, outputs=False
    ):
        """
        ...
        # phenology
        self.phenopara = cpara.phenology
        ...
        self.wmax = cpara.interception.wmax
        self.wmaxsnow = cpara.interception.wmaxsnow
    ...
    def run_timestep(
        ...
        Rn = (
            np.maximum(2.57 * self.LAI / (2.57 * self.LAI + 0.57) - 0.2, 0.55) * Rg
    ...
    def dry_canopy_et(
        ...
        fQ = (
            1.0
            / kp
            * np.log((kp * Qp + q50) / (kp * Qp * np.exp(-kp * self.LAI) + q50 + eps))
        )
        ...
        """ --- transpiration rate --- """
        Tr = penman_monteith((1.0 - tau) * AE, 1e3 * D, Ta, Gc, 1.0 / Ra, units="mm")
        Tr[Tr < 0] = 0.0
        ...
        Efloor[self.SWE > 0] = (
            0.0  # no evaporation from floor if snow on ground or beta == 0
        )
    ...
    def canopy_water_snow(self, dt, T, Prec, AE, D, Ra=25.0, U=2.0):
        ...
        erate[ixr] = (
            ...
            / Lv[ixr]
            * penman_monteith(
                (1.0 - tau[ixr]) * AE[ixr], 1e3 * D[ixr], T[ixr], gs, Ga[ixr], units="W"
            )
        )
