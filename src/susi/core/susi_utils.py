...
def peat_hydrol_properties(x, unit="g/cm3", var="bd", ptype="A"):
    ...
    para["bd"] = {
        "pF0": (97.95, -79.72, 0.0),
        "pF1.5": (20.83, 759.69, -2484.3),
        "pF2": (3.81, 705.13, -2036.2),
        "pF3": (9.37, 241.69, -364.6),
        "pF4": (-0.06, 249.8, -519.9),
        "pF4.2": (0.0, 174.48, -348.9),
    }
    para["H"] = {
        "pF0": (95.17, -1.26, 0.0),
        "pF1.5": (46.20, 8.32, -0.54),
        "pF2": (27.03, 8.14, -0.43),
        "pF3": (17.59, 3.22, -0.07),
        "pF4": (8.81, 3.03, -0.10),
        "pF4.2": (5.8, 2.27, -0.08),
    ...
    intp_pF1["bd"] = interp1d(
        [0.04, 0.08, 0.1, 0.2], [63.0, 84.0, 86.0, 80.0], fill_value="extrapolate"
    )
    intp_pF1["H"] = interp1d(
        [1.0, 4.0, 6.0, 10.0], [75.0, 84.0, 86.0, 80.0], fill_value="extrapolate"
    )
    ...
    vg_ini = (
        ...
        0.03,
        1.3,
    )  # initial van Genuchten parameters (porosity, residual water content, alfa, n)

    x = np.array(x)
    prs = para[var]
    pF1 = intp_pF1[var]
    if unit == "kg/m3" and var == "db":
        x = x / 1000.0
...
def CWTr(nLyrs, z, dz, pF, Ksat, direction="positive"):
    ...
    if direction == "positive":
        sto = [
            sum(wrc(pF, x=np.minimum(z - g, 0.0)) * dz) for g in gwl
        ]  # equilibrium head m
        storoot = [
            np.sum(wrc(pF, x=np.minimum(z - g, 0.0))[0:nroot] * dz[0:nroot])
            for g in gwl
        ]
        storoot2 = [
            np.sum(wrc(pF, x=np.minimum(z - g, 0.0))[0:nroot2] * dz[0:nroot2])
            for g in gwl
        ]
    else:
        sto = [
    ...
    afproot = (storoot2[0] - storoot2) / (
        sum(dz[:nroot2])
    )  # air-filled porosity in root layer
    ratio = (
        airroot[1:] / airtot[1:]
    )  # share of air-filled porosity in rooting zone to total air volume
    sto = list(sto)
    gwl = list(gwl)
    ratio = list(ratio)
    afproot = list(afproot)
    sto.reverse()
    gwl.reverse()
    ratio.reverse()
    afproot.reverse()
    ...
    C = interp1d(
        np.array(gwl),
        np.array(np.gradient(gwlToSto(gwl)) / np.gradient(gwl)),
        fill_value="extrapolate",
    )  # storage coefficient function

    del gwl, sto, ratio, afproot

    # ----------Transmissivity-------------------
    K = np.array(Ksat * 86400.0)  # from m/s to m/day
...
def wrc(pF, x=None, var=None):
    ...
    if type(pF) is dict:  # dict input
        ...
        Ts = np.array(pF["ThetaS"].values())
        Tr = np.array(pF["ThetaR"].values())
        alfa = np.array(pF["alpha"].values())
        n = np.array(pF["n"].values())
        m = 1.0 - np.divide(1.0, n)
    elif type(pF) is list:  # list input
        pF = np.array(
            pF, ndmin=1
        )  # ndmin=1 needed for indexing to work for 0-dim arrays
        Ts = pF[0]
        Tr = pF[1]
        alfa = pF[2]
        n = pF[3]
...
def z_distrib_decomposition(
    gwl, Qdistrib_beta=0.93
):  # Alkuperäinen 0.88, Koukkujoki 0.87
    """
    ...
    """
    return Qdistrib_beta**0.0 - Qdistrib_beta ** (np.abs(gwl * 100.0))
...
def potential_peat_heterotrophic_respiration(T, sfc, V):
    ...
    Rref = (
        0.28 + V * 5.9 * 10**-4 + bd[sfc] * 1000.0 * 9.1 * 10**-4 + wtm * 1.5 * 10**-3
    ...
    Rref = (
        0.0695 + V * 3.7 * 10**-4 + bd[sfc] * 1000.0 * 5.4 * 10**-4 + wtm * 1.2 * 10**-3
    )  # Parameters: Table 3 RHet
    TASmr = 10.4  # mean temperature may-october, deg C
    pt = 99.0  # peat layer thickness, cm
    B = (
        156.032 + 16.5 * TASmr - 0.196 * pt + 0.354 * bd[sfc]
    )  # Ojanen 2010 et al. Table 4
...
def nutrient_release(sfc, sfc_specification, co2release, N=None, P=None, K=None):
    ...
    Nd = {
        ...
        4: {1: 1.4, 2: 1.4},
        5: {1: 1.2, 2: 1.2},
    }  # Mese study: N cont in OM % dm
    Pd = {
        2: {1: 0.1, 2: 0.1},
        3: {1: 0.08, 2: 0.08},
        4: {1: 0.06, 2: 0.06},
        5: {1: 0.05, 2: 0.05},
    }  # Mese study: P cont in OM % dm
    Kd = {
        2: {1: 0.045, 2: 0.045},
        3: {1: 0.04, 2: 0.038},
        4: {1: 0.037, 2: 0.034},
        5: {1: 0.03, 2: 0.03},
    }  # Mese study: P cont in OM % dm (alkuperäinen)
    ...
    if N is None:
        N = Nd[sfc][sfc_specification]
    if P is None:
        P = Pd[sfc][sfc_specification]
    if K is None:
        K = Kd[sfc][sfc_specification]
    ...
    Nt = (
        co2release * CO2_to_C / C_in_OM * N / 100.0 * (1.0 - Nmicrob)
    )  # + Ndeposition   # Nrelease kg ha-1 day-1
    Pt = (
        co2release * CO2_to_C / C_in_OM * P / 100.0 * (1.0 - Pmicrob)
    )  # +  Pdeposition # Prelease kg ha-1 day-1
    Kt = (
        co2release * CO2_to_C / C_in_OM * K / 100.0 * (1.0 - Kmicrob)
    )  # + Kdeposition  # Prelease kg ha-1 day-1

    return Nt, Pt, Kt


def diff_nutrient_release(
    co2release,
    vol,
    spara,
    wpara,
    ...
    for c in range(np.shape(co2release)[0]):
        ...
        Nt = (
            ...
            * CO2_to_C
            / C_in_OM
            * N[sfc][sfc_specification]
            / 100.0
            * (1.0 - Nmicrob)
        )  # Nrelease kg ha-1 day-1
        Pt = (
            co2
            * CO2_to_C
            / C_in_OM
            * P[sfc][sfc_specification]
            / 100.0
            * (1.0 - Pmicrob)
        )  # Prelease kg ha-1 day-1
        Kt = (
            co2
            * CO2_to_C
            / C_in_OM
            * K[sfc][sfc_specification]
            / 100.0
            * (1.0 - Kmicrob)
        )  # Prelease kg ha-1 day-1
        print(
            "Released nutrients N",
            np.round(sum(Nt), 2),
            "P",
            np.round(sum(Pt), 2),
            "K",
    ...
    for c in range(comparisons)[1:]:
        ...
        Nrel = (
            diff * CO2_to_C / C_in_OM * N[sfc] / 100.0 * (1.0 - Nmicrob)
        )  # Nrelease kg ha-1 day-1
        Prel = (
            diff * CO2_to_C / C_in_OM * P[sfc] / 100.0 * (1.0 - Pmicrob)
        )  # Prelease kg ha-1 day-1
        Krel = (
            diff * CO2_to_C / C_in_OM * K[sfc] / 100.0 * (1.0 - Kmicrob)
        )  # Prelease kg ha-1 day-1
        ...
        if c == range(comparisons)[-1]:
            ...
            gr_response = np.array(gr_response)
            ditch_depth = np.array(ditch_depth)
            gr_response = np.insert(gr_response, 0, 0.0)
            fwt = interp1d(
                summer_dwt, ditch_depth, bounds_error=False, fill_value="extrapolate"
            )
            fgr = interp1d(
                ditch_depth, gr_response, bounds_error=False, fill_value="extrapolate"
            )
            cr_depth = fwt(0.35)
            gr_crd = fgr(fwt(0.35))
        else:
            cr_depth = 0
            gr_crd = 0
        susi_io.write_gr_excel(
            wlocation, wpara, spara, outpara, gN, gP, gK, c, cr_depth, gr_crd
        )
        ...
        if po:
            ...
            plt.fill_between(
                dates, vol, vol + np.cumsum(Krel * gK), color="red", alpha=0.5
            )
...
def critical_ditch_depth(summer_dwt, ditch_depth, growth_response):
    ...
    fgr = interp1d(
        ditch_depth, growth_response, bounds_error=False, fill_value="extrapolate"
    )
...
def nut_to_vol(
    ...
    gvshare = (
        gv_leafmass / (leafmass + gv_leafmass)
    )  # maximum share of ground vegetation from the nutrient uptake -> shared in proportion of green biomass
    ...
    par = {  # Palviainen & Finer, 2012 Eur J For Res 131:945-964, eq 2, Table 7
        "N": {
            "Pine": [1.856, 0.631, 0.050],
            "Spruce": [2.864, 0.557, 0.051],
            "Birch": [1.590, 0.788, 0.044],
        },
        "P": {
            "Pine": [-0.202, 0.566, 0.115],
            "Spruce": [1.109, 0.461, 0.051],
            "Birch": [-1.034, 0.838, 0.162],
        },
        "K": {
            "Pine": [0.839, 0.650, 0.078],
            "Spruce": [2.487, 0.478, 0.061],
            "Birch": [-0.811, 1.087, 0.055],
        },
    }

    # litter_nut={'N':12.0, 'P':1.0, 'K':3.5}      #mg/g Laiho 1997 page 49
    litter_nut = {
        "Pine": {"N": 6.08, "P": 0.63, "K": 2.29},  # mg/g Palviainen & Finer 2012
    ...
    litterN = litter_mass * (1.0 - retrans["N"]) * litter_nut[species]["N"] / 1000.0
    litterP = litter_mass * (1.0 - retrans["P"]) * litter_nut[species]["P"] / 1000.0
    litterK = litter_mass * (1.0 - retrans["K"]) * litter_nut[species]["K"] / 1000.0
...
def motti_development(spara, ifile):
    ...
    if sp < 3:
        if spe != species_codes[sp]:
            print(
                "Species mismatch: check Motti simulation and species code in stand parameters"
            )
    ...
    longevityBranch = {"Pine": 22.0, "Spruce": 22.0, "Birch": 22.0}  # Pine Mäkinen 1999
    ...
    x = np.insert(df["age"].values, 0, 0.0)
    h = np.insert(df["hdom"].values, 0, 0.0)
    ...
    lmass = np.insert(df["leaves"].values, 0, 0.0)  # tn / ha
    bmToLeafMass = interp1d(
        bm, lmass, fill_value=(lmass[0], lmass[-1]), bounds_error=False
    )
    bmToLAI = interp1d(
        bm,
        lmass * sla[spe] / 10.0,
        fill_value=(lmass[0] * sla[spe] / 10.0, lmass[-1] * sla[spe] / 10.0),
        bounds_error=False,
    )
    ...
    bmStemlike = (
        rho[spe] * df["yield"].values
        + (df["stump"].values + df["roots_coarse"].values) * 1000.0
    ...
    bmCrownlike = (
        ...
        + df["leaves"].values
        + df["roots_fine"].values
    ) * 1000.0  # crownlike biomass components for nutrient computation kg/ha
    bmStemlike = np.insert(bmStemlike, 0, 0.0)
    bmCrownlike = np.insert(bmCrownlike, 0, 0.0)
    ageToStemlike = interp1d(
        x, bmStemlike, fill_value=(bmStemlike[0], bmStemlike[-1]), bounds_error=False
    )
    ageToCrownlike = interp1d(
        x, bmCrownlike, fill_value=(bmCrownlike[0], bmCrownlike[-1]), bounds_error=False
    )
    ...
    ageToLeaves = interp1d(
        x, bmLeaves, fill_value=(bmLeaves[0], bmLeaves[-1]), bounds_error=False
    )
    ageToFineRoots = interp1d(
        x, bmFineRoots, fill_value=(bmFineRoots[0], bmFineRoots[-1]), bounds_error=False
    )
    ageToBranch = interp1d(
        x, bmBranch, fill_value=(bmBranch[0], bmBranch[-1]), bounds_error=False
    )
    litter = (
        ageToLeaves(a_arr) / longevityLeaves[spe] * np.gradient(a_arr)
        + ageToFineRoots(a_arr) / longevityFineRoots[spe] * np.gradient(a_arr)
        + ageToBranch(a_arr) / longevityBranch[spe] * np.gradient(a_arr)
    )  # Litterfall kg/ha in timestep
    litter2 = litter.copy()
    litter2 = np.insert(litter2, 0, 0.0)
    bm2 = ageToBm(a_arr)
    bm2 = np.insert(bm2, 0, 0.0)
    bmToLitter = interp1d(
        bm2,
        litter2 / 365.25,
        fill_value=(litter2[0] / 365.25, litter2[-1] / 365.25),
        bounds_error=False,
    )  # output now in daily litterfall

    N = (
        ageToStemlike(a_arr) * nuts[spe]["Stem"]["N"] * 1e-3
        + ageToCrownlike(a_arr) * nuts[spe]["Crown"]["N"] * 1e-3
    )  # kg
    P = (
        ageToStemlike(a_arr) * nuts[spe]["Stem"]["P"] * 1e-3
        + ageToCrownlike(a_arr) * nuts[spe]["Crown"]["P"] * 1e-3
    )
    K = (
        ageToStemlike(a_arr) * nuts[spe]["Stem"]["K"] * 1e-3
        + ageToCrownlike(a_arr) * nuts[spe]["Crown"]["K"] * 1e-3
    )
    Nlit = np.cumsum(litter) * nuts[spe]["Crown"]["N"] * 1e-3 * (1.0 - retrans["N"])
    Plit = np.cumsum(litter) * nuts[spe]["Crown"]["P"] * 1e-3 * (1.0 - retrans["P"])
    Klit = np.cumsum(litter) * nuts[spe]["Crown"]["K"] * 1e-3 * (1.0 - retrans["K"])
    ...
    print(
        fill,
        "Vol as yield",
        np.round(ageToYield(a_arr[0]), 2),
        np.round(ageToYield(a_arr[-1]), 2),
    )
...
def assimilation(ppara, rg, vpd, Ta_minus1, Ta, rew, LAI, Xk, Ns, Ps, Ks, hdom):
    ...
    fr = interp1d(
        ...
        np.array([0.6, 0.50, 0.36, 0.20]),
        fill_value="extrapolate",
    )  # Mäkelä & Valentine 2001 Tree Physiology 21, 1015-1030, Härkönen et al. 2010 ForEco 259, 524-533

    NPP = Pk * fr(hdom) * 2.0  # Change from gC m-2 to g organic matter

    return NPP * 10.0  # kg/ha organic matter
...
def assimilation_yr(ppara, dfforc, wt, afp, LAI, LAI_above):
    ...
    for m in range(ncols):
        par[m, 0:days] = (
            rg * 4.6 * 0.5 / 1000000.0 * 86400.0 * attenuation[m]
        )  # Unit conversion to mol/m2/day, 0.5 is the share of par from rg
    ...
    for column in range(ncols):
        ...
        Pk_pot = (
            ...
            * 1.0
            * 1.0
        )  # growth that is not restricted by edaphic factors gC m-2 day-1

        """Respi"""
        NPP = Pk * 0.5 * 2.0
        NPP_pot = Pk_pot * 0.5 * 2.0  # Change from gC m-2 to g organic matter
...
def heterotrophic_respiration_yr(t5, yr, dfwt, v, spara):
    ...
    T5zero = -46.02
    T5ref = 10.0
    T5 = np.minimum(T5, 16.0)
    Rref = (
        0.0695 + 3.7 * 10 ** (-4) * v + 5.4 * 10 ** (-4) * bd + 1.2 * 10 ** (-3) * wt
    ) * 24.0  # unit g/m2/h CO2-> g/m2/day
    Rhet = [
        rref * np.exp(B * ((1.0 / (T5ref - T5zero)) - (1.0 / (T5 - T5zero))))
        for rref in Rref
    ]
    ...
    return (
        days,
        np.mean(Rhet, axis=1) * 10.0,
        np.sum(Rhet, axis=0) * 10.0,
...
def ojanen_2019(spara, yr, dfwt):
    ...
    if sfc < 3:
        soil_co2_balance = (
            (-115 + 12 * wts) * 10 * -1
        )  # conversion from g/m2/yr to kg co2/ha/ yr and sign conversion: source negative; sink positive
    else:
...
def CH4_flux_yr(yr, dfwt):
    ...
    CH4 = (
        -0.378 + 12.3 * np.exp(-0.121 * wt)
    ) * 10.0  # Ojanen et al. 2010, Fig. 6, convert to kg CH4 /ha/year
    ...
    return (
        ...
        CH4mean,
        CH4mean * 54.0,
    )  # convert to kg CO2-eq./ha/year, SGWP100 coefficient = 54 (Ojanen et al. 2021)


def understory_uptake(
    n,
    lat,
    lon,
    barea0,
    ...
    dem = np.ones(n) * 80.0  ## x2 surface elevation m asl
    ...
    def gv_biomass_and_nutrients(
        ...
        fl_share = {
            "description": "share of dwarf shrubs (ds) and herbas & grasses (h) from field layer biomass, kg kg-1",
            "pine_upland": {"ds": 0.91, "h": 0.09},
            "spruce_upland": {"ds": 0.71, "h": 0.29},
            "broadleaved_upland": {"ds": 0.38, "h": 0.62},
            "spruce_mire": {"ds": 0.50, "h": 0.50},
            "pine_bog": {"ds": 0.90, "h": 0.10},
        }
        nut_con = {
            "description": "nutrient concentration of dwarf shrubs (ds), herbs & grasses (h), upland mosses (um), and sphagna (s), unit mg/g",
            "ds": {"N": 12.0, "P": 1.0, "K": 4.7},
            "h": {"N": 18.0, "P": 2.0, "K": 15.1},
            "um": {"N": 12.5, "P": 1.4, "K": 4.3},
            "s": {"N": 6.0, "P": 1.4, "K": 4.3},
        }
        lit_share = {
            "description": "share of living biomass that is lost as litter annually for dwarf shrubs (ds), herbs & grasses (h), upland mosses (um), and sphagna (s), unit: kg kg-1",
            "ds": 0.2,
            "h": 0.5,
            "um": 0.3,
            "s": 0.3,
        ...
        green_share = {
            "description": "share of green mass from the total biomass of dwarf shrubs (ds), herbs & grasses (h), upland mosses (um), and sphagna (s), unit kg/kg",
            "ds": 0.2,
            "h": 0.5,
            "um": 0.3,
            "s": 0.3,
        ...
        retrans = {
            "description": "share of nutrients retranslocated before litterfallfor dwarf shrubs (ds), herbs & grasses (h), upland mosses (um), and sphagna (s), unit: kg kg-1",
            "ds": {"N": 0.69, "P": 0.73, "K": 0.87},
            "h": {"N": 0.69, "P": 0.73, "K": 0.87},
            "um": {"N": 0.0, "P": 0.0, "K": 0.0},
            "s": {"N": 0.0, "P": 0.0, "K": 0.0},
        }
        ...
        gv_tot[ix] = (
            np.square(
                35.52
                + 0.001 * longitude * dem[ix]
                - 1.1 * drain_s**2
                - 2e-5 * vol[ix] * Nstems[ix]
                + 4e-5 * Nstems[ix] * age[ix]
                + 0.139 * longitude * drain_s
            )
            - 0.5
            + 116.54
        )  # Total, Eq.39, Table 9
        gv_bot[ix] = (
            np.square(
                -3.182
                + 0.022 * latitude * longitude
                + 2e-4 * dem[ix] * age[ix]
                - 0.077 * sfc[ix] * longitude
                - 0.003 * longitude * vol[ix]
                + 2e-4 * np.square(vol[ix])
            )
            - 0.5
            + 98.10
        )  # Bottom layer total, Eq. 35, Table 9
        gv_field[ix] = (
            np.square(
                23.24
                - 1.163 * drain_s**2
                + 1.515 * sfc[ix] * drain_s
                - 2e-5 * vol[ix] * Nstems[ix]
                + 8e-5 * ts * age[ix]
                + 1e-5 * Nstems[ix] * dem[ix]
            )
            - 0.5
            + 162.58
        )  # Field layer total, Eq. 37, Table 9
        ...
        ds_litterfall[ix] = (
            fl_share["spruce_mire"]["ds"]
            * (gv_tot[ix] - gv_bot[ix])
            * lit_share["ds"]
            * fl_to_total_turnover
        )
        h_litterfall[ix] = (
            fl_share["spruce_mire"]["h"]
            * (gv_tot[ix] - gv_bot[ix])
            * lit_share["h"]
            * fl_to_total_turnover
        )
        ...
        gv_tot[ix] = (
            np.square(
                50.098
                + 0.005 * longitude * dem[ix]
                - 1e-5 * vol[ix] * Nstems[ix]
                + 0.026 * sfc[ix] * age[ix]
                - 1e-4 * dem[ix] * ts
                - 0.014 * vol[ix] * drain_s
            )
            - 0.5
            + 167.40
        )  # Total, Eq 45, Table 9
        gv_bot[ix] = (
            np.square(
                31.809
                + 0.008 * longitude * dem[ix]
                - 3e-4 * Nstems[ix] * ba[ix]
                + 6e-5 * Nstems[ix] * age[ix]
                - 0.188 * dem[ix]
            )
            - 0.5
            + 222.22
        )  # Bottom layer total, Eq 41, Table 9
        gv_field[ix] = (
            np.square(
                48.12
                - 1e-5 * ts**2
            ...
            - 0.5
            + 133.26
        )  # Field layer total, Eq. 43, Table 9
        ...
        s_litterfall[ix] = gv_bot[ix] * lit_share["s"]
        gv_leafmass[ix] = (
            fl_share["pine_bog"]["ds"] * (gv_tot[ix] - gv_bot[ix]) * green_share["ds"]
    ...
    nup = (
        nup_net + nup_litter_mean * simtime
    )  # total N uptake kg ha-1 simulation time (in yrs) -1
    pup = (
        pup_net + pup_litter_mean * simtime
    )  # total P uptake kg ha-1 simulation time (in yrs) -1
    kup = (
        kup_net + kup_litter_mean * simtime
    )  # total P uptake kg ha-1 simulation time (in yrs) -1
