...
class MossLayer(object):
    ...
    def __init__(self, org_para_array: OrganicLayerParametersArray, outputs=False):
        ...
        # and field capacity
        self.dz_top = org_para_array.org_depth  # depth, m3 m-3
        self.poros_top = org_para_array.org_poros  # porosity, m3 m-3
        self.fc_top = org_para_array.org_fc  # field capacity m3 m-3
        self.rw_top = org_para_array.org_rw  # ree parameter m3 m-3
        self.Wsto_top_max = self.fc_top * self.dz_top  # maximum storage m

        # pond storage
        self.h_pond_max = org_para_array.pond_storage_max

        # initialize state
        # toplayer storage and relative conductance for evaporation
        self.Wsto_top = self.Wsto_top_max * org_para_array.org_sat
        self.Wliq_top = self.poros_top * self.Wsto_top / self.Wsto_top_max
        self.Ree = np.maximum(
            0.0, np.minimum(0.98 * self.Wliq_top / self.rw_top, 1.0)
        )  # relative evaporation rate (-)
        # pond storage
        self.h_pond = org_para_array.pond_storage
    ...
    def interception(self, potinf=0.0, evap=0.0):
        ...
        self.mbe = (
            (pond_ini - self.h_pond)
            + (Wsto_top_ini - self.Wsto_top)
            + potinf0
            - potinf
            - evap
        )
