...
def mass(ff, scen):
    ...
    litter = (
        ncf["groundvegetation"]["ds_litterfall"][scen, :, :] / 10000.0
        + ncf["groundvegetation"]["h_litterfall"][scen, :, :] / 10000.0
        + ncf["groundvegetation"]["s_litterfall"][scen, :, :] / 10000.0
        + ncf["stand"]["nonwoodylitter"][scen, :, :] / 10000.0
        + ncf["stand"]["woodylitter"][scen, :, :] / 10000.0
    )

    soil = ncf["esom"]["Mass"]["out"][scen, :, :] / 10000.0 * -1 + litter
    ...
    for sto in esoms[7:]:
        inipeat += ncf["esom"]["Mass"][sto][scen, 0, :] / 10000.0
    endpeat = np.zeros(cols)
    for sto in esoms[7:]:
        endpeat += ncf["esom"]["Mass"][sto][scen, -1, :] / 10000.0
    inimor = np.zeros(cols)
    for sto in esoms[:7]:
        inimor += ncf["esom"]["Mass"][sto][scen, 0, :] / 10000.0
    endmor = np.zeros(cols)
    for sto in esoms[:7]:
        endmor += ncf["esom"]["Mass"][sto][scen, -1, :] / 10000.0
    ...
    gvlitter = (
        ncf["groundvegetation"]["ds_litterfall"][scen, :, :]
    ...
    for c in range(cols):
        peat = np.zeros(yrs)
        for sto in esoms[7:]:
            peat += ncf["esom"]["Mass"][sto][scen, :, c] / 10000.0
        mor = np.zeros(yrs)
        for sto in esoms[:7]:
            mor += ncf["esom"]["Mass"][sto][scen, :, c] / 10000.0
    ...
    # ---soil mass change----------
    out = ncf["esom"]["Mass"]["out"][scen, :, :] / 10000.0 * -1 + litter
...
def nutrient_balance(ff, substance, scen):
    ...
    # -------------- Total supply ----------------------------------------
    supply = (
        ncf["balance"][substance]["decomposition_root_lyr"][scen, :, :]
...
def compare_1(ff, scens):
    ...
    deltawt = (
        ncf["strip"]["dwtyr"][scens[1], :, :] - ncf["strip"]["dwtyr"][scens[0], :, :]
    )
    ax = fig.add_subplot(gs[10:, 8:])
    ax = create_profile_boxplot(
    ...
    # -------soil C balance kg/ha/yr---------------------------
    litter0 = (
        ncf["groundvegetation"]["ds_litterfall"][scens[0], :, :] / 10000.0
        + ncf["groundvegetation"]["h_litterfall"][scens[0], :, :] / 10000.0
        + ncf["groundvegetation"]["s_litterfall"][scens[0], :, :] / 10000.0
        + ncf["stand"]["nonwoodylitter"][scens[0], :, :] / 10000.0
        + ncf["stand"]["woodylitter"][scens[0], :, :] / 10000.0
    ) * mass_to_c

    soil0 = (
        ncf["esom"]["Mass"]["out"][scens[0], :, :] / 10000.0 * -1 * mass_to_c + litter0
    )
    ...
    litter1 = (
        ncf["groundvegetation"]["ds_litterfall"][scens[1], :, :] / 10000.0
        + ncf["groundvegetation"]["h_litterfall"][scens[1], :, :] / 10000.0
        + ncf["groundvegetation"]["s_litterfall"][scens[1], :, :] / 10000.0
        + ncf["stand"]["nonwoodylitter"][scens[1], :, :] / 10000.0
        + ncf["stand"]["woodylitter"][scens[1], :, :] / 10000.0
    ) * mass_to_c

    soil1 = (
        ncf["esom"]["Mass"]["out"][scens[1], :, :] / 10000.0 * -1 * mass_to_c + litter1
    )
    ...
    # ------------Site C balance----------------------
    gv = ncf["groundvegetation"]["gv_tot"][scens[0], :, :] / 10000.0 * mass_to_c
    grgv0 = np.diff(gv, axis=0)
    stand = ncf["stand"]["biomass"][scens[0], :, :] / 10000.0 * mass_to_c
    gr0 = np.diff(stand, axis=0)
    out0 = ncf["esom"]["Mass"]["out"][scens[0], :, :] / 10000.0 * -1 * mass_to_c
    ...
    gv = ncf["groundvegetation"]["gv_tot"][scens[1], :, :] / 10000.0 * mass_to_c
    grgv1 = np.diff(gv, axis=0)
    stand = ncf["stand"]["biomass"][scens[1], :, :] / 10000.0 * mass_to_c
    gr1 = np.diff(stand, axis=0)
    out1 = ncf["esom"]["Mass"]["out"][scens[1], :, :] / 10000.0 * -1 * mass_to_c
    ...
    ax = fig.add_subplot(gs[:2, 8:])
    ax = create_profile_boxplot(
        ax, deltasoil, cols, "green", "Site C difference", "", fs, facecolor, zero=True
    )
...
def compare_runs(ff_0, ff_1, scen):
    ...
    # -------soil C balance kg/ha/yr---------------------------
    litter0 = (
        ncf_0["groundvegetation"]["ds_litterfall"][scen, :, :] / 10000.0
        + ncf_0["groundvegetation"]["h_litterfall"][scen, :, :] / 10000.0
        + ncf_0["groundvegetation"]["s_litterfall"][scen, :, :] / 10000.0
        + ncf_0["stand"]["nonwoodylitter"][scen, :, :] / 10000.0
        + ncf_0["stand"]["woodylitter"][scen, :, :] / 10000.0
    ) * mass_to_c

    soil0 = (
        ncf_0["esom"]["Mass"]["out"][scen, :, :] / 10000.0 * -1 * mass_to_c + litter0
    )
    ...
    litter1 = (
        ncf_1["groundvegetation"]["ds_litterfall"][scen, :, :] / 10000.0
        + ncf_1["groundvegetation"]["h_litterfall"][scen, :, :] / 10000.0
        + ncf_1["groundvegetation"]["s_litterfall"][scen, :, :] / 10000.0
        + ncf_1["stand"]["nonwoodylitter"][scen, :, :] / 10000.0
        + ncf_1["stand"]["woodylitter"][scen, :, :] / 10000.0
    ) * mass_to_c

    soil1 = (
        ncf_1["esom"]["Mass"]["out"][scen, :, :] / 10000.0 * -1 * mass_to_c + litter1
    )
    ...
    # ------------Site C balance----------------------
    gv = ncf_0["groundvegetation"]["gv_tot"][scen, :, :] / 10000.0 * mass_to_c
    grgv0 = np.diff(gv, axis=0)
    stand = ncf_0["stand"]["biomass"][scen, :, :] / 10000.0 * mass_to_c
    gr0 = np.diff(stand, axis=0)
    out0 = ncf_0["esom"]["Mass"]["out"][scen, :, :] / 10000.0 * -1 * mass_to_c
    ...
    gv = ncf_1["groundvegetation"]["gv_tot"][scen, :, :] / 10000.0 * mass_to_c
    grgv1 = np.diff(gv, axis=0)
    stand = ncf_1["stand"]["biomass"][scen, :, :] / 10000.0 * mass_to_c
    gr1 = np.diff(stand, axis=0)
    out1 = ncf_1["esom"]["Mass"]["out"][scen, :, :] / 10000.0 * -1 * mass_to_c
...
def compare_scens(ff):
    def draw_comparison(
        ax, x, y, sd, label, ylabel, title, color, facecolor, fs, xlabel=False
    ):
        ax.plot(x, y, "o-", color=color, label=label)
        ax.set_ylabel(ylabel, fontsize=fs)
        if xlabel:
            ax.set_xlabel("Ditch depth, m", fontsize=fs)
    ...
    grresponse = (
        ncf["stand"]["volumegrowth"][:, :, :] - ncf["stand"]["volumegrowth"][0, :, :]
    )
    ...
    standco2bal = np.mean(
        ncf["balance"]["C"]["stand_c_balance_co2eq"][:, :, :], axis=(1, 2)
    )
    standco2balsd = np.std(
        ncf["balance"]["C"]["stand_c_balance_co2eq"][:, :, :], axis=(1, 2)
    )
    ...
    soilco2bal = np.mean(
        ncf["balance"]["C"]["soil_c_balance_co2eq"][:, :, :], axis=(1, 2)
    )
    soilco2balsd = np.std(
        ncf["balance"]["C"]["soil_c_balance_co2eq"][:, :, :], axis=(1, 2)
    )
