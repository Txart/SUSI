...
class Gvegetation:
    def __init__(self, n, lat, lon, sfc, species):
        ...
        self.lit_share = {
            ...
            "um": 0.3,
            "s": 0.3,
        }  # see Ojanen et al 2013 Forest Ecology and Management 289 (2013) 201–208
        ...
        self.n = n
        self.sfc = sfc
        self.dem = np.ones(n) * 80.0  ## x2 surface elevation m asl
    ...
    def reset_domain(self):
        ...
        self.gv_change = np.zeros(
            self.n
        )  # biomass change during time step kg ha-1 yr-1
        ...
        self.h_litterfall = np.zeros(
            self.n
        )  # herbs and grasses litterfall kg ha-1 yr-1
    ...
    def gv_biomass_and_nutrients(self, ts, vol, Nstems, ba, age):
        ...
        gv_tot[ix] = (
            np.square(
                35.52
                + 0.001 * self.longitude * self.dem[ix]
                - 1.1 * self.drain_s**2
                - 2e-5 * vol[ix] * Nstems[ix]
                + 4e-5 * Nstems[ix] * age[ix]
                + 0.139 * self.longitude * self.drain_s
            )
            - 0.5
            + 116.54
        )  # Total, Eq.39, Table 9
        gv_bot[ix] = (
            np.square(
                -3.182
                + 0.022 * self.latitude * self.longitude
                + 2e-4 * self.dem[ix] * age[ix]
                - 0.077 * self.sfc[ix] * self.longitude
                - 0.003 * self.longitude * vol[ix]
        ...
        gv_field[ix] = (
            np.square(
                23.24
                - 1.163 * self.drain_s**2
                + 1.515 * self.sfc[ix] * self.drain_s
                - 2e-5 * vol[ix] * Nstems[ix]
                + 8e-5 * ts * age[ix]
                + 1e-5 * Nstems[ix] * self.dem[ix]
            )
            - 0.5
            + 162.58
        ...
        ds_litterfall[ix] = (
            self.fl_share["spruce_mire"]["ds"]
            * (gv_tot[ix] - gv_bot[ix])
            * self.lit_share["ds"]
            * self.fl_to_total_turnover
        )
        h_litterfall[ix] = (
            self.fl_share["spruce_mire"]["h"]
            * (gv_tot[ix] - gv_bot[ix])
            * self.lit_share["h"]
            * self.fl_to_total_turnover
        )
        ...
        n_litter_nw[ix] = (
            ...
            * self.nut_con["ds"]["N"]
            * 1e-3
            * (1.0 - self.retrans["ds"]["N"])
            * 0.25
            + h_litterfall[ix]
            * self.nut_con["h"]["N"]
            * 1e-3
            * (1.0 - self.retrans["h"]["N"])
            + s_litterfall[ix]
            * self.nut_con["s"]["N"]
            * 1e-3
            * (1.0 - self.retrans["s"]["N"])
        )
        ...
        p_litter_nw[ix] = (
            ...
            * self.nut_con["ds"]["P"]
            * 1e-3
            * (1.0 - self.retrans["ds"]["P"])
            * 0.25
            + h_litterfall[ix]
            * self.nut_con["h"]["P"]
            * 1e-3
            * (1.0 - self.retrans["h"]["P"])
            + s_litterfall[ix]
            * self.nut_con["s"]["P"]
            * 1e-3
            * (1.0 - self.retrans["s"]["P"])
        )
        ...
        k_litter_nw[ix] = (
            ...
            * self.nut_con["ds"]["K"]
            * 1e-3
            * (1.0 - self.retrans["ds"]["K"])
            * 0.25
            + h_litterfall[ix]
            * self.nut_con["h"]["K"]
            * 1e-3
            * (1.0 - self.retrans["h"]["K"])
            + s_litterfall[ix]
            * self.nut_con["s"]["K"]
            * 1e-3
            * (1.0 - self.retrans["s"]["K"])
        )
        ...
        gv_tot[ix] = (
            np.square(
                50.098
                + 0.005 * self.longitude * self.dem[ix]
                - 1e-5 * vol[ix] * Nstems[ix]
                + 0.026 * self.sfc[ix] * age[ix]
                - 1e-4 * self.dem[ix] * ts
                - 0.014 * vol[ix] * self.drain_s
            )
            - 0.5
            + 167.40
        )  # Total, Eq 45, Table 9
        gv_bot[ix] = (
            np.square(
                31.809
                + 0.008 * self.longitude * self.dem[ix]
        ...
        gv_field[ix] = (
            np.square(
                48.12
                - 1e-5 * ts**2
                + 0.013 * self.sfc[ix] * age[ix]
                - 0.04 * vol[ix] * self.drain_s
                + 0.026 * self.sfc[ix] * vol[ix]
            )
            - 0.5
            + 133.26
        ...
        ds_litterfall[ix] = (
            self.fl_share["pine_bog"]["ds"]
            * (gv_tot[ix] - gv_bot[ix])
            * self.lit_share["ds"]
            * self.fl_to_total_turnover
        )
        h_litterfall[ix] = (
            self.fl_share["pine_bog"]["h"]
            * (gv_tot[ix] - gv_bot[ix])
            * self.lit_share["h"]
            * self.fl_to_total_turnover
        )
    ...
    def run(self, ba, stems, vol, sp, ts, age):
        ...
        (
            ...
            s_litterfall,
            gv_leafmass,
        ) = self.gv_biomass_and_nutrients(ts, vol, stems, ba, age)
